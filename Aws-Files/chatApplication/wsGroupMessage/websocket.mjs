...
import { getConnectionsInRoom, deleteConnection } from './database.mjs';
...
// Returns promises to send messages to all connected clients in a room
export async function getSendMessagePromises(message, roomId, domain, stage) {
    // Get connection IDs of clients in the room
    let clientIdArray = await getConnectionsInRoom(roomId);
    console.log("\nClient IDs in room " + roomId + ":\n" + JSON.stringify(clientIdArray));
